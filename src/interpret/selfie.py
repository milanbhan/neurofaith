            ...
            # Prepare the model inputs for generation
                ...
                for _ in range(self.max_new_tokens):
                    ...
                    with torch.no_grad():
                        ...
                        if interpreter_input_masks is None:
                            # Create causal mask: [1, 1, seq_len, seq_len]
                            interpreter_input_masks = torch.tril(
                                torch.ones((1, 1, seq_len, seq_len), device=self.model.device)
                            ).to(dtype=torch.bool)

                        # ✅ 4. Precompute RoPE (cos, sin) — only once
                        cos, sin = self.model.model.rotary_emb(interpreter_hidden_states, position_ids=interpreter_position_ids)
                        ...
                        for i in range(k, len(layers)):
                            ...
                            interpreter_hidden_states = layers[i](
                                ...
                                attention_mask=interpreter_input_masks,
                                position_ids=interpreter_position_ids,
                                position_embeddings=position_embeddings,  # safe in 4.51.0
                                )[0]
                        interpreter_hidden_states = self.model.model.norm(interpreter_hidden_states)
                        logits = self.model.lm_head(interpreter_hidden_states[:, -1, :])
